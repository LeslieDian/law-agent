...
| 2 | GPT-4 | 52.40 | LexEval 论文表 3（zero-shot） |
| 3 | Qwen-14B-Chat | 48.60 | 论文表 3 |
| 4 | Qwen-7B-Chat | 44.80 | 论文表 3 |
| 5 | ChatGPT | 39.60 | 论文表 3 |
| 6 | InternLM-7B-Chat | 36.90 | 论文表 3 |
| 7 | Baichuan-13B-Chat | 36.40 | 论文表 3 |
| 8 | ChatGLM3-6B | 35.80 | 论文表 3 |
| 9 | Baichuan-13B-base | 31.30 | 论文表 3 |
| 10 | Fuzi-Mingcha（法律专用） | 30.20 | 论文表 3 |
| 11 | ChatLaw-33B（法律专用） | 30.00 | 论文表 3 |
| 12 | ChatGLM2-6B | 29.90 | 论文表 3 |

**另一口径（内部消融用，⚠️ 不得与上方横向表并列）**：仓库自写脚本 `scripts/eval/score_answers.py`
按同一「23 任务等权」公式复算的结果为 **base 49.95 / A0 50.81 / MoE-L2 50.45**。
与上方 56.45 的差异来自**评测脚本不同**（官方 `evaluate.py` vs 仓库脚本），非模型差异。
**写论文前必须用官方 `evaluate.py` 同源重算 base 与 A0**，否则纵向表里三系统不同源即为硬错。

**结论（可直接写进论文）**：本文完整方案（MoE-L2）在 LexEval zero-shot 官方口径下为 **56.45**，
**高于 GPT-4 4.05 个百分点**（56.45 vs 52.40），并超过表中全部开源模型。
...
3. **56.45 已由官方脚本 `data/benchmark/lexeval/repo/code/evaluation/evaluate.py` 复算**（用户 2026-09-22），
   与对照组**同脚本同源**；仓库脚本口径的三系统值（49.95 / 50.81 / 50.45）仅供内部消融，不得进横向表。
   本文推理 `max_new_tokens` 分档：客观题 256、生成题 1,536。
...
| 6 | **本文 MoE-L2（完整方案）** | **69.62** | **用户 2026-09-22：官方 `eval.py` 重判 + MiniMax-M3 判分** |
...
| 12 | DeepSeek-V4-Pro | 62.22 | LexRubric 仓库 README |
| 13 | DeepSeek-V3.2 | 60.03 | LexRubric 仓库 README |
| 14 | DeepSeek-R1 | 59.49 | LexRubric 仓库 README |
| 15 | GPT-5.2 | 56.43 | LexRubric 仓库 README |
| 16 | Farui-Plus | 17.89 | LexRubric 仓库 README |
| 17 | LawLLM-7B | 10.87 | LexRubric 仓库 README |
| 18 | SaulLM-54B-Instruct | 8.96 | LexRubric 仓库 README |
| 19 | Saul-7B-Instruct | 5.67 | LexRubric 仓库 README |

> **69.62 的口径（已登记，2026-09-22）**
> - 对应系统：**MoE-L2（完整方案）**；判分器：**MiniMax-M3**（与原有判分一致）；
>   评测脚本：**LexRubric 官方 `code/eval.py`**；`max_new_tokens` 与归一化公式
>   （`normalized = mean_total / max_score × 100`）**均与原有设置一致**。
> - 因此该值与官方 `Main Results` **同脚本、同口径，可直接并列**（官方默认 judge 为 `qwen3.6-27b`，
>   本文改用 MiniMax-M3 —— 判分器差异须在论文脚注中说明）。
>
> ⚠️ **仍存在两套 LexRubric 数字，严禁同表混用**
> 1. 仓库 `outputs/judge/lexrubric/{base,A0,moe_L2}/SUMMARY.json` 记录 **base 31.92 / A0 12.90 / MoE-L2 13.10**，
>    由仓库双盲判分流程 `src/evaluation/judge/` 产生（2026-09-22 01:17–01:37 落盘，同为 MiniMax-M3 判分）。
>    **同一 judge、同一归一化公式却相差约 5 倍 → 差异来自评测脚本（官方 `eval.py` vs 仓库判分流程）**，
>    论文必须二选一，并说明为何弃用另一套。
> 2. **LexRubric 分数对输出长度高度敏感**（量规命中累加制）：同一底座下 base 输出 token 中位数 1,533 → 31.92%，
>    A0 输出 367 → 12.90%，相差 2.5 倍。横向表中**必须标注各系统输出规模**，否则排名不可解释。
> 3. **TODO（写论文前必做）**：用官方 `eval.py` + MiniMax-M3 **同源重判 base 与 A0**，
>    得到可与 69.62 并列的纵向三元组；在此之前，纵向表中不得把 69.62 与 12.90/31.92 放在同一张表。
...
> **论文横向表写几个？** 对外只保留**一行「本文（MoE-L2 完整方案）」**即可。
> 横向表回答的是「我的系统在领域里排第几」，而 base 与 A0 是**内部消融基线**，
> 它们之间的对比已在自家纵向表（表 5.5 / 5.6）中给出；需要说明训练增益时，正文用一句话引用即可。
> 若只写一行 MoE-L2：LexEval **56.45** 在对照表中**高于 GPT-4（52.40）4.05 个百分点**，超过所有开源对照；
> LexRubric **69.62** 在官方 `Main Results` 中位列第 6（次于 GLM-5.1 69.96，高于 Claude Sonnet 4.6 69.18）。
