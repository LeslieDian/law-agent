...
2. 样本唯一键 = `content_sha1`，**全局唯一（0 重复）**，构造成立即断言。
   ★ 池规模**不在此写死**：它随上游变化（阶段 2 找回误删样本、阶段 3 去污口径、
   阶段 2c 派生量都会动），实测值一律见运行报告 `docs/corpus/SPLIT_STATS.json`。
   2026-09-18 起阶段 2 的 `uid` 已修为
...
def render_md(args, stats, path):
    ...
    with open(path, "w", encoding="utf-8") as fh:
        ...
        W("- ⚠️ 阶段 7 所需的「路由评测集 **200–500 条人工标注**」**必须新建**，"
          "不在本阶段产物内（原表述写「贴近 CLaw 254 案分布」—— CLaw 已于 "
          "2026-09-19 退役，分布依据改为**内部验证集**的域/任务分布）。")
        W()
