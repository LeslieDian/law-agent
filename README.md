...
4. [评测基准](#4-评测基准)
   - 4b [与公开模型成绩的横向对照](#4b-与公开模型成绩的横向对照)
5. [目录结构](#5-目录结构)
6. [实验环境](#6-实验环境)
7. [数据与合规（强制）](#7-数据与合规强制)
8. [进度与执行顺序](#8-进度与执行顺序)
   - [终评结果（官方脚本口径 · 论文主表唯一数据源）](#终评结果官方脚本口径--论文主表唯一数据源--2026-09-22-定稿)
9. [结果完整性要求](#9-结果完整性要求)
10. [文档维护约定](#10-文档维护约定)
...
> 本节是**论文「与其他模型对比」章节的唯一数据源**，两张表**口径不同、禁止合并成一个分数**。
> 对照组成绩**直接引用基准论文/仓库公布的表格**，本文成绩由本仓库已有产物按**同一口径重算** —— **不需要再跑 GPU**。
> 引用到论文时必须连同下方「口径警示」一起保留。

### 4b-1 LexEval：23 任务等权平均（zero-shot，%）

LexEval 论文表 3 的 `Average` 列 = **23 个任务等权平均**（19 个客观任务取 Accuracy，4 个生成任务
`5_1`–`5_4` 取 ROUGE-L），**不是题目等权**。本文按同一口径从 `outputs/score/lexeval_{base,A0,moe_L2}.json`
重算：客观任务取 `accuracy.by_task[*].exact`，生成任务取 `rouge_l.by_split["5_*"].f_mean`，23 项等权平均后 ×100。

| 排名 | 模型 | Average (%) | 来源 |
|---:|---|---:|---|
| 1 | **本文 MoE-L2（层内软混合，完整方案）** | **56.45** | **用户 2026-09-22 用官方 `repo/code/evaluation/evaluate.py` 复算** |
| 2 | **本文 A0（统一适配器，内部基线）** | **54.36** | 用户 2026-09-22 同脚本复算（同脚本同源） |
| 3 | GPT-4 | 52.40 | LexEval 论文表 3（zero-shot） |
| 4 | **本文 base（Qwen3-8B 4-bit，无微调，内部基线）** | **50.20** | 用户 2026-09-22 同脚本复算（同脚本同源） |
| 5 | Qwen-14B-Chat | 48.60 | 论文表 3 |
| 6 | Qwen-7B-Chat | 44.80 | 论文表 3 |
| 7 | ChatGPT | 39.60 | 论文表 3 |
| 8 | InternLM-7B-Chat | 36.90 | 论文表 3 |
| 9 | Baichuan-13B-Chat | 36.40 | 论文表 3 |
| 10 | ChatGLM3-6B | 35.80 | 论文表 3 |
| 11 | Baichuan-13B-base | 31.30 | 论文表 3 |
| 12 | Fuzi-Mingcha（法律专用） | 30.20 | 论文表 3 |
| 13 | ChatLaw-33B（法律专用） | 30.00 | 论文表 3 |
| 14 | ChatGLM2-6B | 29.90 | 论文表 3 |

**另一口径（内部消融与显著性检验用，⚠️ 不得与上方横向表并列）**：仓库自写脚本 `scripts/eval/score_answers.py`
按同一「23 任务等权」公式复算的结果为 **base 49.95 / A0 50.81 / MoE-L2 50.45**（2026-09-22 已全部落袋，非待办）。
与官方口径的差异来自**评分脚本实现不同**（官方 `evaluate.py` vs 仓库脚本），非模型差异。

⚠️ **两套脚本给出相反的序关系（写论文前必须知道）**
- 官方口径：MoE-L2 **56.45** > A0 **54.36** > base **50.20**
- 仓库口径：A0 **50.81** > MoE-L2 **50.45** > base **49.95**
→ **论文主表一律用官方口径**（与对照组同脚本同源）；仓库口径只用于内部消融与配对显著性检验，
且引用时须注明其序关系与官方口径不一致。

**结论（可直接写进论文 · 2026-09-22 定稿）**
- 完整方案（MoE-L2）在 LexEval zero-shot 官方口径下为 **56.45**，**高于 GPT-4 4.05 个百分点**（56.45 vs 52.40），
  并超过表中全部开源模型；
- 统一适配器 A0 为 **54.36**，仍**高于 GPT-4 1.96 个百分点**；未微调底座 base 为 **50.20**，低于 GPT-4 2.20 个百分点；
- 三系统**同脚本同源**，纵向增益可直接写正文：MoE-L2 − A0 **+2.09 pp**、A0 − base **+4.16 pp**、MoE-L2 − base **+6.25 pp**。

⚠️ 该结论强度很高，论文中必须完整交代判定脚本（官方 `evaluate.py`）、ROUGE-L 实现与推理配置，
否则极易被质疑；同时须说明本文系统为 8B 量级、4-bit 量化，与对照组参数量级不同。

**逐任务增益（A0 − base，> +4 pp 的任务）**：`3_3` +19.30、`3_5` +12.60、`5_1` +10.99、
`5_4` +5.61、`2_5` +4.40；明显下滑：`1_2` −21.20、`1_1` −4.20。
<sub>⚠️ 逐任务数字由**仓库脚本** `scripts/eval/score_answers.py` 产出，与官方 `evaluate.py` 不同源；
论文若以官方口径为主表，此段宜改写为「按任务族（客观 / 生成）层面的趋势」或不列出逐任务数值。</sub>

**四条必须带上的脚注**：
1. 对照组成绩引自 LexEval 论文（arXiv:2409.20288）表 3 的 zero-shot `Average` 列，**非同期、非同硬件评测**。
2. 本文系统为 Qwen3-8B **4-bit 量化** + QLoRA 适配器（8B 量级），对照组含 13B/14B/33B 与闭源模型，**参数量级不同**。
3. **三系统值 56.45 / 54.36 / 50.20 已全部由官方脚本 `data/benchmark/lexeval/repo/code/evaluation/evaluate.py` 复算**
   （用户 2026-09-22），与对照组**同脚本同源**；仓库脚本口径（50.45 / 50.81 / 49.95）仅供内部消融与显著性检验，
   不得进横向表。本文推理 `max_new_tokens` 分档：客观题 256、生成题 1,536。
4. 本文只做 zero-shot，故只与论文表 3 对照；论文表 5（few-shot）不参与比较。

### 4b-2 LexRubric：score rate（%）

官方 `Main Results`（score rate %）与本仓库实测：

| 排名 | 模型 | Overall (%) | 来源 |
|---:|---|---:|---|
| 1 | Kimi K2.6 | 75.21 | LexRubric 仓库 README |
| 2 | Qwen3.6-Max-Preview | 75.12 | LexRubric 仓库 README |
| 3 | Qwen3-Max | 74.35 | LexRubric 仓库 README |
| 4 | Kimi K2.5 | 73.40 | LexRubric 仓库 README |
| 5 | GLM-5.1 | 69.96 | LexRubric 仓库 README |
| 6 | **本文 MoE-L2（层内软混合，完整方案）** | **69.62** | **用户 2026-09-22：官方 `eval.py` 重判 + MiniMax-M3 判分** |
| 7 | Claude Sonnet 4.6 | 69.18 | LexRubric 仓库 README |
| 8 | Qwen3.5-397B-A17B | 68.03 | LexRubric 仓库 README |
| 9 | LegalOne-8B（法律专用 8B） | 65.14 | LexRubric 仓库 README |
| 10 | GLM-5 | 63.28 | LexRubric 仓库 README |
| 11 | DeepSeek-V4-Flash | 63.19 | LexRubric 仓库 README |
| 12 | **本文 A0（统一适配器，内部基线）** | **62.95** | 用户 2026-09-22：官方 `eval.py` + MiniMax-M3（同口径） |
| 13 | DeepSeek-V4-Pro | 62.22 | LexRubric 仓库 README |
| 14 | **本文 base（Qwen3-8B 4-bit，无微调，内部基线）** | **61.25** | 用户 2026-09-22：官方 `eval.py` + MiniMax-M3（同口径） |
| 15 | DeepSeek-V3.2 | 60.03 | LexRubric 仓库 README |
| 16 | DeepSeek-R1 | 59.49 | LexRubric 仓库 README |
| 17 | GPT-5.2 | 56.43 | LexRubric 仓库 README |
| 18 | Farui-Plus | 17.89 | LexRubric 仓库 README |
| 19 | LawLLM-7B | 10.87 | LexRubric 仓库 README |
| 20 | SaulLM-54B-Instruct | 8.96 | LexRubric 仓库 README |
| 21 | Saul-7B-Instruct | 5.67 | LexRubric 仓库 README |

> **官方 `eval.py` 口径（已登记 · 2026-09-22 · 论文主表唯一数据源）**
> - 三系统：**MoE-L2 69.62 / A0 62.95 / base 61.25**（用户 2026-09-22 用 LexRubric 官方 `code/eval.py` 重判，同源完成，TODO 已闭合）。
> - 判分器 **MiniMax-M3**（与仓库原有判分一致，**非**官方默认 `qwen3.6-27b`）；`max_new_tokens` 与归一化公式
>   （`normalized = mean_total / max_score × 100`）**均与原有设置一致**。
> - 因此三值与官方 `Main Results` **同脚本、同口径，可并列**；判分器差异须在论文脚注中说明。
> - 三值同源 → 可作论文的纵向三元组：MoE-L2 − A0 **+6.67 pp**、A0 − base **+1.70 pp**、MoE-L2 − base **+8.37 pp**。
>
> ⚠️ **仍存在两套 LexRubric 数字，严禁同表混用（论文二选一：官方口径）**
> 1. **仓库脚本口径**：`outputs/judge/lexrubric/{base,A0,moe_L2}/SUMMARY.json` 记录
>    **base 31.92 / A0 12.90 / MoE-L2 13.10**（仓库双盲判分流程 `src/evaluation/judge/`，2026-09-22 01:17–01:37 落盘，
>    同为 MiniMax-M3 判分）。**同一 judge、同一归一化公式却相差 2–5 倍 → 差异来自评测脚本**
>    （官方 `eval.py` vs 仓库判分流程）。
> 2. **序关系同样反转**：官方口径 MoE-L2 69.62 > A0 62.95 > base 61.25（微调后更好）；
>    仓库口径 base 31.92 反而最高、A0 12.90 最低。→ 论文采用官方口径，并在脚注说明仓库口径为
>    **另一评测脚本实现**，不作为主结果。
> 3. **长度效应解释不成立**：仓库口径下「base 输出 token 中位数 1,533 → 31.92%、A0 输出 367 → 12.90%」
>    这一「输出变短导致分数下降」的解释，在官方口径下不复现（base 61.25 **低于** A0 62.95）
>    → **论文不得把长度效应作为主结果的解释**。
> 4. 横向表须标注本文系统为 **8B 量级 4-bit 量化**，与表中大参数/闭源模型不同量级。

### 4b-3 三个系统的含义与论文取舍

| 名称 | 是什么 | 论文里的角色 |
|---|---|---|
| **base** | 原始 Qwen3-8B（4-bit 量化，**不挂任何 LoRA**） | 起点基线，回答「微调到底带来什么」 |
| **A0** | 统一适配器：100k 四域混合语料上训练的**单个** QLoRA（r16/α32，43.6M 可训练参数） | 消融基线（「不分域」的对照组）；同时是 MoE 的第 4 个专家（兜底） |
| **MoE-L2** | 4 个专家 LoRA（civil / criminal / procedure / unified=A0，**全部冻结**）+ 每层可学习门控（36 层 × 2 = 72 个门控，≈2.36M 参数），只训门控 | **论文主方法 / 交付系统** |

> **论文横向表写几个？** 默认只保留**一行「本文（MoE-L2 完整方案）」**；
> 若需要展示训练增益，**可再加 A0 / base 两行** —— 三者已**同源同脚本**（官方 `evaluate.py` / 官方 `eval.py`），同表并列无硬错。
>
> | 指标（官方脚本口径） | base | A0 | **MoE-L2** | 论文用法 |
> |---|---:|---:|---:|---|
> | LexEval Average（%） | 50.20 | 54.36 | **56.45** | 横向表主行只写 MoE-L2；纵向增益可写三行 |
> | LexRubric score rate（%） | 61.25 | 62.95 | **69.62** | 同上 |
>
> 横向表回答的是「我的系统在领域里排第几」；base 与 A0 同时是**内部消融基线**。
> 配对显著性检验结论（基于**仓库**脚本口径，序关系与官方口径不一致，引用需注明）见
> `D:\统一文件夹\论文\内部集_三专家vsA0_按域切片核查_20260922.md`。
> 若只写一行 MoE-L2：LexEval **56.45** 在对照表中**高于 GPT-4（52.40）4.05 个百分点**，超过所有开源对照；
> LexRubric **69.62** 在官方 `Main Results`（21 个系统）中位列**第 6**，高于 LegalOne-8B（65.14）与 DeepSeek-R1（59.49），
> 低于 GLM-5.1（69.96）。
...
| P3 | A0 全量基准生成 | GPU0 分档链：lexrubric 649 (cap1536) → lexeval 客观 11,400 (cap256) → lexeval 生成 2,750 (cap1536)，单系统 ≈14h | ✅ 已完成（2026-09-22 三系统全量均落袋）；2026-09-20 20:24 以**口径对齐后代码**重启（旧口径 504 条已归档 `answers.bf16norm_STALE.jsonl`） |
| P4 | A0 判分 | LexEval 客观秒级本地打分；LexRubric 走 `--judges minimax-m3`（12,335 条 rubric / 系统） | ✅ 已完成（三系统 649/649，2026-09-22 01:37 落盘） |
...
| P9 | 消融 + 终评 | E0/E5 主对比 + A1/A3/A4（oracle/平均/随机路由）+ A7/A8/A9；各系统按 P3 分档口径生成、按 P4 判分；完整矩阵双卡 ≈1.75–2.6 天 | 🔶 **终评已完（数字见官方口径表）**；**A1/A3/A4 + A7/A8/A9 消融未跑** |
...
## 剩余路线与时间表（历史存档 · 2026-09-21 10:10 定稿）

> ⚠️ **本节为 2026-09-21 的历史路线图存档，实际已于 2026-09-22 全部执行完毕**（阶段进度表 21/21 ✅）。
> **写论文不要参考本节的时间预估**，请直接看「终评结果（官方脚本口径）」与 §4b 横向对照表。
...
- **留空待填的部分（只留空白，其余照常）**：
  1. ✅ **主表 9 格已全部落袋**（A0 / MoE / base × LexRubric / 客观 Acc / 生成 ROUGE-L）——
     仓库口径见「终评结果（自动汇总）」，**论文主表改用「终评结果（官方脚本口径）」三元组**；
  2. 🔶 **MoE 消融数字**：门控路由准确率已有独立集数字 top1_acc **0.8370** / macro_f1 **0.7922** /
     回退率 0.18 / routed_acc 0.8976（可直接写）；**A1(oracle)/A3(平均)/A4(随机) + A7/A8/A9 终评侧未跑**
     → 论文只能写「混合不退化」，不能写「增益来自路由」；
  3. **损失曲线 / 域配比 / 检索消融等图**——图已在 `docs/figures/` 陆续产出，可直接先用；
  4. ✅ **三专家内部集域专业化分析段已补齐**（2026-09-22）：内部 test 1,000 条按域切片，
     criminal 300 / civil 400 / procedural 200 / general 100；结论见
     `D:\统一文件夹\论文\内部集_三专家vsA0_按域切片核查_20260922.md`
     ——**禁止写「专家弱于统一模型」**，域内基本持平（±0.01），差距全在非本域与通用域。
- ✅ **数字已全部落袋（2026-09-22 15:00 前）**：横向对照见 §4b，纵向三元组见「终评结果（官方脚本口径）」。
  **可以开始写论文，无需再等任何实验。**
...
- 判分口径、cap 限制、ROUGE/归一化公式已固化：
  **论文主表用「终评结果（官方脚本口径）」**（官方 `evaluate.py` / 官方 `eval.py` + MiniMax-M3 判分）；
  「终评结果（自动汇总）」为仓库脚本口径，仅作内部消融与显著性检验，实验设置章引用时需二选一并注明。
...
### 终评结果（官方脚本口径 · 论文主表唯一数据源 · 2026-09-22 定稿）

> 上方「终评结果（自动汇总）」由 `scripts/eval/collect_results.py` **按仓库脚本**每小时自动回填（勿手改）；
> 其 LexRubric 列（31.92 / 12.90 / 13.10）与 LexEval 列（49.95 / 50.81 / 50.45）与官方口径
> **数值相差数倍且序关系相反** → **严禁抄进论文主表**。论文一律使用下表。

| 系统 | LexRubric score rate（官方 `eval.py`，%） | LexEval Average（官方 `evaluate.py`，%） | 论文角色 |
|---|---:|---:|---|
| base（Qwen3-8B 4-bit，无微调） | 61.25 | 50.20 | 起点基线 |
| A0（统一适配器 QLoRA） | 62.95 | 54.36 | 消融基线（不分域对照） |
| **MoE-L2（4 专家 LoRA + 72 门控，完整方案）** | **69.62** | **56.45** | **主方法 / 交付系统** |

- **纵向增益（同源同脚本，可直接写进正文）**：
  LexRubric — MoE-L2 − A0 **+6.67 pp**、A0 − base **+1.70 pp**、MoE-L2 − base **+8.37 pp**；
  LexEval — MoE-L2 − A0 **+2.09 pp**、A0 − base **+4.16 pp**、MoE-L2 − base **+6.25 pp**。
- **判分器为 MiniMax-M3**（非 LexRubric 官方默认 judge）→ 论文脚注必须说明。
- **横向对照表见 §4b**（LexEval 表 / LexRubric 表，对照组引自官方论文与仓库 README）。
- 内部验证集（1,000 条）与三域专家的对比**只有仓库脚本口径**（`docs/eval/RESULTS_MATRIX.md`），
  **不得与官方口径混表**；其中生成题 ROUGE-L 的全量 14,150 口径（0.5626 / 0.0934 / 0.2050）
  被客观题字母答案污染，**干净口径只取 2,750 道生成题 = 0.2595 / 0.3100 / 0.3180**。
- ⚠️ **仍未跑的消融**：A1(oracle) / A3(平均) / A4(随机路由) + A7/A8/A9 —— 无法证明增益来自「路由」本身，
  只能证明「混合不退化」。这是审稿必问项，论文讨论节须如实列为局限。
...
- ✅ **阶段 6–8 已完成（2026-09-22）**：L2 门控自检 + 训练（72 gates / 2.36M 参数，1125/1125 步，→ `gate_weights.pt`）→
  MoE 等价验证（与 peft 单适配器 `max|Δ|=0` 逐位一致）→ 终评（LexRubric 649 + LexEval 14,150，三系统全部跑完）。
- ⬜ **阶段 9 消融未跑**：A1(oracle) / A3(平均) / A4(随机路由) + A7/A8/A9 —— **论文最大缺口**：
  无法证明增益来自「路由」本身，只能证明「混合不退化」。论文讨论节须如实列为局限。
