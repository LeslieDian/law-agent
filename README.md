...
| 排名 | 模型 | Average (%) | 来源 |
|---:|---|---:|---|
| 1 | GPT-4 | 52.40 | LexEval 论文表 3（zero-shot） |
| 2 | **本文 A0（统一适配器）** | **50.81** | 本仓库产物，同口径重算 |
| 3 | **本文 MoE-L2（层内软混合，完整方案）** | **50.45** | 本仓库产物，同口径重算 |
| 4 | **本文 base（Qwen3-8B 未微调）** | **49.95** | 本仓库产物，同口径重算 |
...
**结论（可直接写进论文）**：本文三个系统在 LexEval zero-shot 口径下均**超过表中全部开源模型
（含 14B/33B 量级与法律专用模型）**，与 GPT-4 的差距为 **1.59 个百分点**（50.81 vs 52.40）。
...
3. 本文推理 `max_new_tokens` 分档：客观题 256、生成题 1,536；生成任务 ROUGE-L 为**字符级 LCS（cap=800，β=1.2）**，
   与官方实现可能不同 → **放表前须用官方脚本 `data/benchmark/lexeval/repo/code/evaluation/evaluate.py` 复算核对**（纯 CPU，无需 GPU）。
...
| 6 | **本文系统** | **69.62** | **用户 2026-09-22 实测（判分口径待登记，见警示）** |
...
> ⚠️ **口径警示（勿删，论文引用前必须处理）**
> 1. **69.62 与仓库产物不是同一口径。** `outputs/judge/lexrubric/{base,A0,moe_L2}/SUMMARY.json` 记录的
>    `normalized_pct` 为 **base 31.92 / A0 12.90 / MoE-L2 13.10**，判分器为 **MiniMax-M3 单判**
>    （`aggregation = mean_total / max_score × 100`，2026-09-22 01:17–01:37 落盘）。
>    69.62 为用户 2026-09-22 实测值，**判分器、prompt 版本、输出长度上限、归一化公式均待登记**。
>    **两套数字严禁在同一张表内混用**，论文只能选一套。
> 2. **LexRubric 分数对输出长度高度敏感**（量规命中累加制）：同一底座下 base 输出 token 中位数 1,533 → 31.92%，
>    A0 输出 367 → 12.90%，相差 2.5 倍。因此横向表中**必须标注各系统的输出规模**，否则排名不可解释。
> 3. **TODO**：登记 69.62 的判分器 / prompt 哈希 / `max_new_tokens` / 归一化公式 → 回写 `docs/eval/`，
>    并与 12.90 那一套做一次同源对照；完成后删除本警示。
...
> 若只写一行 MoE-L2：LexEval 50.45 在对照表中**仅次于 GPT-4（52.40）**，超过所有开源对照。
