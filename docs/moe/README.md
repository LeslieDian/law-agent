...
## 3-bis. 首批实测结果（2026-09-20）

### L1 路由分类器 —— 全量 20,000 条，已跑完

命令见第 5 节。编码 20,000×1024 用 **263.4s（GPU，75.9 条/s）**；
分类头（逻辑回归 ×4）**12.66s**；**全流程 298.3s**，不值得单独占卡。

| 指标 | 内部 dev（2,002） | **独立集 test 1k** |
|---|---|---|
| exact_match | 0.8257 | **0.8150** |
| micro_f1 / macro_f1 | 0.8577 / 0.8109 | **0.8342 / 0.7922** |
| top1_acc | 0.8776 | **0.8370** |
| fallback_rate | 0.1553 | 0.1800 |
| routed_acc（接受单路由那部分的准确率） | 0.9320 | **0.8976** |
| routed_coverage | 0.8447 | 0.8200 |

独立集逐类 F1：`criminal 0.9221` > `civil 0.8629` > `procedure 0.7024` > `unified 0.6812`。

**怎么读这张表**：

- **回退策略是有效的，不是摆设**：18% 的样本被判为「低置信 / 跨域」而回退给 unified，
  这部分**不计入**路由准确率；剩下 82% 里准确率 **0.8976** —— 比一刀切的 0.8370 高 6 个点。
  这正是 `adapters_router.yaml` 里"置信度<0.6 回退"那条规则第一次有了实测支撑。
- **最弱的是 `procedure`（recall 0.59）**：程序法问题大量被判成 civil / criminal。
  这与第 1.3 节的判断一致 —— 程序法本来就渗透在民事/刑事里，法域边界最糊。
  **这条恰好是 L2 软混合最该补的场景**：硬选必须二选一，软混合可以让权重摊在
  `procedure` 与 `civil` 两个专家上。
- `unified` 的 precision 只有 0.60 —— 兜底专家被过度召唤。门控训出来（L2）之后，
  这一步由权重决定而不是阈值规则，预期会改善。

### L2 等价性自检 —— 已 PASS

CPU / 4 层 / fp32，专家 = `A0_unified_qwen3_8b` + `_smoke_criminal`：

| 检查 | 结果 |
|---|---|
| 专家 1（A0）one-hot 等价 | `max abs diff = 0`，相对 0.000e+00 → **bit-exact PASS** |
| 专家 2（_smoke_criminal）one-hot 等价 | `max abs diff = 0` → **bit-exact PASS** |
| 装配 | 替换 28 个模块、建 8 个门控、K=2、形状不符 **0** |
| 门控可训练参数 | 131,088（0.0065%） |
| 门控参数张量独立性 | 8 个门控 / 16 个独立张量（应 = 16）✓ |
| 逐层输出差异 | 同输入下不同层门控输出 > 1 种 ✓ |
| **VERDICT** | **PASS** |

> `max|Δ| = 0` 是逐位相等，不是"在容差内"。fp32 下 peft 的前向与我们的前向走同一串算子，
> 因此**相等是应当的**；一旦 A/B 转置、缩放或加权有任何一处写错，差值会立刻跳到
> LoRA 增量的量级（远大于 0）。所以这个 0 本身就是最强的证据。
